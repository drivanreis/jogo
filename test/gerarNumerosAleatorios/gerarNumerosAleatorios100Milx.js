...
const numTestes = 100000; // Altere este valor para 1000, 10000, etc.

// Função para testar a geração de números 'numTestes' vezes
function testarGerarNumerosAleatorios() {
    let todasSequencias = [];
    let totalSequenciasRepetidas = 0; // Novo contador para o total de sequências repetidas
    ...
    if (Object.keys(repeticoes).length === 0) {
        ...
        for (const sequencia in repeticoes) {
            totalSequenciasRepetidas += repeticoes[sequencia] - 1; // Conta as repetições, excluindo a primeira ocorrência
            console.log(`A sequência [${sequencia}] foi sorteada ${repeticoes[sequencia]} vezes.`);
        }
    }

    // Exibe o total de sequências repetidas
    console.log(`\nTotal de sequências repetidas: ${totalSequenciasRepetidas}`);
}
